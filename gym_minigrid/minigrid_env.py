...
class MiniGridEnv(ABC):  # (gym.Env):
    ...
    def __init__(
        ...
        # Global planner for the environment. The planner finds
        # the shortest obstacle free map from the agent to the goal.
        # It can be used to provide short-term waypoints for the 
        # agent.
        self.use_global_planner = use_global_planner
        self.global_planner = GlobalPlanner(
                                use_carrot_stick = use_carrot_stick_waypoint,
                                waypoint_dist=waypoint_distance)

        # If set to true the global plan will be updated at each frame. 
        # Otherwise a single plan will be generated upon reset
        self.static_plan = not use_carrot_stick_waypoint
        
        # The planned path by the global planner 
        self.global_plan = []

        # List of waypoints generated by the global planner
        self.waypoints = []
    ...
    def step(self, action):
        ...
        # Move forward
            ...
            if fwd_cell and fwd_cell.type == 'waypoint':
                # Remove the waypoint upon reaching it
                self.grid.set(*fwd_pos, None)

                # TODO: We probably want ot provide small rewards
    ...
    def reset(self):
        ...
        if self.use_global_planner:
            self.global_planner.load_map(self.grid)
            self.global_planner.update_agent(self.agent_pos)
            self.global_planner.update_goal()
            self.global_planner.replan()
            self.global_plan = self.global_planner.get_path()
            self.waypoints = self.global_planner.get_waypoints()
    ...
    def render(self, mode='rgb_array', close=False, highlight=True, visualize_plan = True, tile_size=TILE_PIXELS):
